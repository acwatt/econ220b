...
# Need to merge over actual replacement mileage from data
replacement_dates %>%
    mutate(mileage_replace1 = ifelse(!is.na(replace_date1), 
                                     filter(data, bus_number==bus_number, date==replace_date1)$mileage,
                                     NA))
...
data = data %>%
    ...
    left_join(replacement_dates %>% select(bus_number,
                                           ...
                                           replace_date2,
                                           engine_replacement_1_odometer_reading,
                                           engine_replacement_2_odometer_reading,
                                           group),
              by = 'bus_number') %>%
...
4. Reset the Mileage whenver replacements are made

```{r}
data = data %>%
    arrange(bus_number, date)

data %>%
    group_by(bus_number) %>%
    mutate(mileage2 = case_when(date < replace_date1 & !is.na(replace_date1) ~ mileage))

# For each bus
for (bus in unique(data$bus_number)) {
    data[]
}
```
